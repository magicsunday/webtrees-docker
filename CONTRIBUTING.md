...
- Issue / PR titles in absolute terms: `Probe Windows host IP under
  WSL2`, not `Like NathanVaughn does it`. Commit-message subject
  starts with a capitalised verb (`Add`, `Probe`, `Derive`); no
  conventional-commits prefixes (`feat:` / `chore:` / `fix:`).
- Commit messages reference the issue via `GH-<N>:` prefix on the
  subject line.

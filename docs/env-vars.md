...
| `MAIL_SMTP` | none (entrypoint skips msmtprc render) | `compose.yaml`, `rootfs/docker-entrypoint.sh` | SMTP relay host with optional port (`relay.example.org:587`). When set, the entrypoint writes `/etc/msmtprc` so PHP's `mail()` → msmtp routes through it. Empty/unset leaves the file absent; webtrees' built-in SMTP transport (Control panel → Site settings → Email) bypasses msmtp entirely and remains available either way. |
| `MAIL_DOMAIN` | `.env.dist` ships `example.org` | `compose.yaml`, `rootfs/docker-entrypoint.sh` | Domain for the envelope From address msmtp uses when the message body doesn't carry one. Renders as `from webtrees@${MAIL_DOMAIN}`. Unset → msmtp omits the `from` directive. |
| `MAIL_HOST` | none | `compose.yaml`, `rootfs/docker-entrypoint.sh` | HELO/EHLO identity sent to the SMTP relay. Set to a forward-resolvable hostname when the relay enforces strict HELO. Unset → msmtp omits the `domain` directive and uses its built-in fallback. |
...
* **`MAIL_*` block** — names align with common SMTP-relay conventions (`MAIL_SMTP=host:port`, `MAIL_DOMAIN`, `MAIL_HOST`). The entrypoint's `setup_mail` step renders them into `/etc/msmtprc`; PHP's `mail()` reaches the relay via msmtp's `sendmail`-compatible binary wired through `php.ini`'s `sendmail_path`. Operators wanting a TLS-pinned or auth'd relay bind-mount their own `/etc/msmtprc`. webtrees' built-in SMTP transport (Control panel → Site settings → Email) bypasses msmtp entirely and remains the recommended path for production relays.

...
| `DOCKER_SERVER` | `ghcr.io/magicsunday` | `compose.yaml`, `compose.development.yaml`, `Dockerfile` | Registry namespace for all `${DOCKER_SERVER}/webtrees-*` image pulls (flat canonical names; the legacy `${DOCKER_SERVER}/webtrees/*` form stays published as a deprecation alias). |

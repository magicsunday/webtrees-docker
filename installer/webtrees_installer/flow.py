...
# Local fallback copy of the admin password. The init container reads the
# canonical password from the secrets volume; this file is the operator-facing
# backup when they scrolled past the install banner. Mentioned in the banner,
# referenced by tests — keep the single source of truth here.
ADMIN_PASSWORD_BACKUP_FILENAME = ".webtrees-admin-password"
...
def _write_admin_password_secret(*, work_dir: Path, password: str) -> None:
    ...
    _write_secret_file(work_dir / ADMIN_PASSWORD_BACKUP_FILENAME, password)
...
def _print_banner(
    *,
    ...
    if admin_user is not None:
        ...
        print("╚══════════════════════════════════════════════════════════════╝", file=stdout)
        print(
            "The password is autogenerated — rotate after first login "
            "(Control panel → Users).",
            file=stdout,
        )
        print(
            f"Backup copy: {ADMIN_PASSWORD_BACKUP_FILENAME} (mode 0600). "
            "Delete once rotated.",
            file=stdout,
        )

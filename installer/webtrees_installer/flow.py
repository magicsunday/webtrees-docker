...
def _print_banner(
    *,
    ...
    if admin_user is not None:
        ...
        # Draw the box in the info colour. Each complete line is wrapped
        # AFTER its `:<48` padding so the ANSI escape lands at the line
        # edges only and the column alignment is preserved; colour
        # collapses to plain ASCII on a non-TTY / NO_COLOR stream.
        print(term.info("╔══════════════════════════════════════════════════════════════╗"), file=stdout)
        print(term.info("║                 webtrees admin credentials                   ║"), file=stdout)
        print(term.info("╠══════════════════════════════════════════════════════════════╣"), file=stdout)
        print(term.info(f"║  user:      {admin_user:<48} ║"), file=stdout)
        print(term.info(f"║  password:  {admin_password:<48} ║"), file=stdout)
        print(term.info("╚══════════════════════════════════════════════════════════════╝"), file=stdout)
        print(

...
from webtrees_installer.flow import (
    ADMIN_PASSWORD_BACKUP_FILENAME,
    StandaloneArgs,
    run_standalone,
)
...
def silence_prereq_and_docker(tmp_path_factory, monkeypatch) -> None:
    ...
    with patch("webtrees_installer.flow.check_prerequisites"), \
        ...
        def fake(*, work_dir: Path, password: str) -> None:
            (work_dir / ADMIN_PASSWORD_BACKUP_FILENAME).write_text(password + "\n")
        ws.side_effect = fake
        yield
...
def test_run_standalone_reveals_admin_password(tmp_path: Path) -> None:
    ...
    assert "rotate after first login" in body
    assert ADMIN_PASSWORD_BACKUP_FILENAME in body
...
def test_run_standalone_no_admin_hides_credentials_box(tmp_path: Path) -> None:
    ...
    assert "rotate after first login" not in body
    assert ADMIN_PASSWORD_BACKUP_FILENAME not in body

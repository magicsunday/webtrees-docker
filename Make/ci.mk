...
ci-cache-ignore-error-lockstep: .logo ## Asserts every build.yml `cache-to: type=gha` entry carries ignore-error=true (a cache-export 5xx must not fail the build — GH-187).
	$(call lockstep,cache ignore-error lockstep,check-cache-ignore-error.sh)

...
ci-php-versions-lockstep: .logo ## Asserts dev/versions.json has exactly one row per supported PHP minor per webtrees minor.
	...
	# dev/php-versions.json `.supported`. dev/versions.json carries
	# the actual catalog rows the build matrix consumes; for every
	# unique webtrees minor in the catalog, the set of PHP values
	# MUST equal `supported` exactly (no missing minor, no extra).
	#
	...
	# Actual (per webtrees-minor): sorted unique `.php` values for
	# every row carrying that minor. Compare per-bucket; first drift
	# fails loud with a `::error::` annotation naming the offending
	# webtrees minor + the symmetric-difference set.
	...
	#     precedent (`^(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)$`); a hand-
	#     edited `.supported` that slips a malformed value past this
	#     filter would propagate via the auto-bump cron into every
	#     new versions.json row (check-versions.yml fans the value
	#     verbatim) without any downstream layer catching it.
	...
	#     listed a duplicate.
	supported_raw=$$(docker run --rm -v "$(PWD)/dev:/d:ro" -w /d ghcr.io/jqlang/jq:latest \
			-r '(.supported // []) | length' php-versions.json) || { \
			echo "::error::docker run for supported-php length-probe failed" >&2; \
			exit 1; \
		}; \
		supported_clean=$$(docker run --rm -v "$(PWD)/dev:/d:ro" -w /d ghcr.io/jqlang/jq:latest \
		...
		supported_clean_count=$$(printf '%s' "$$supported_clean" | docker run --rm -i ghcr.io/jqlang/jq:latest -r length) || { \
		...
		supported=$$(printf '%s' "$$supported_clean" | docker run --rm -i ghcr.io/jqlang/jq:latest -r 'sort | join(",")') || { \
		...
		webtrees_minors=$$(docker run --rm -v "$(PWD)/dev:/d:ro" -w /d ghcr.io/jqlang/jq:latest \
			-r '[.[].webtrees] | unique | .[]' versions.json) || { \
			echo "::error::docker run for webtrees-minor extraction failed" >&2; \
			exit 1; \
		}; \
		for wt in $$webtrees_minors; do \
			actual=$$(docker run --rm -v "$(PWD)/dev:/d:ro" -w /d ghcr.io/jqlang/jq:latest \
